...
> Results: **precision 24.8%, recall 57.3%, F1 0.346.** The headline number is the **16.8× lift**: if you alerted on every earthquake, 1.5% would be significant; the events our model flags are **17 times more likely** to be significant, and it still **caught 57%** of them.
...
> And here is the model working on actual future events — its eight highest-confidence alerts. **Four of the eight were verified significant — 50%, versus a 1.5% base rate, 34× better than random.** The misses cluster just below M5.0, exactly where magnitude estimates are least stable. We show the misses on purpose — an honest model earns trust."

### 7:00–8:30 — The response platform  *(poster 09) + live demo*
> "The platform closes the loop: **detect** from the live feed, **score** with the model, **zone** lookup against the risk map, **alert** on-call responders via SMS or app — and **dispatch** pre-positioned resources. Only top-confidence, in-risk-zone events page a human, which cuts alert fatigue while keeping a 57% catch rate.
...
### 9:20–10:00 — Close
> "To sum up: **11,143 real earthquakes, one honest model, a 16.8× lift, and a response pipeline that turns raw seismic data into minutes saved.** We chose to show our misses as proudly as our hits, because in disaster management, trust is the deliverable. Thank you — we're happy to take questions."
...
| Precision / Recall / F1 | **24.8% / 57.3% / 0.346** |
| Lift vs 1.5% baseline | **16.8×** |
| Feature importance | Location **73.5%**, Depth **16.4%**, Hour 6.6%, Day 3.5% |
| Top-8 alerts verified | **4 of 8** (50% vs 1.5% base → 34×) |
...
**Q3. "Explain the 16.8× lift."**
"Precision math. Alert on everything → 1.5% of alerts are real. Our model's alerts → 24.8% are real. 24.8 ÷ 1.5 ≈ 17. So a flagged event is about 17× more likely to be significant than a random one — and we still caught 57% of all significant events. Lift is the fairest single number for imbalanced problems like this."
...
"Precision: of the alarms we raise, how many are real — **1 in 4**. Recall: of the real significant earthquakes, how many we sounded the alarm for — **57%**. For a fire alarm you'd want both near 100%; here we deliberately accept 3 false alarms per real one, because missing a real M6.5 is catastrophic while a false alarm just costs a few minutes of checking."
